...
`when`을 표현식으로 사용하면 값을 반드시 반환해야 하니, 모든 상황에 철저하게 대처해야 한다.

하지만 `enum`을 사용하면 가능한 값을 모두 사용했는지 코틀린이 알 수 있는 경우도 있다.

sealed도 마찬가지로 타입의 개수가 정해져 있다.

else를 사용하지 않았는데 sealed class에 새로운 서브클래스를 추가하면 사용하는 곳에서도 새 타입을 추가해야 한다 -> 무슨 뜻이지?

그래서 공유 모듈에 속할 때는 불편한 상황이 생긴다. when을 사용하는 모든 코드에서 새로운 타입을 추가해야 하기 때문이다.
